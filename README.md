...
**정답지가 없는 OCR 운영환경에서 confidence와 text embedding이 어떤 오류를 잡는지 비교하고, 위험 문서를 먼저 검수하는 방법을 만든 프로젝트**
...
처음에는 OCR confidence가 유용한 baseline이지만 그것만으로는 부족하고, 정상 문서의 의미 공간에서 벗어나는 정도를 함께 보면 놓친 오류를 더 찾을 수 있을 것으로 예상했습니다. 실험 결과는 더 복합적이었습니다. Confidence가 예상보다 강했고 embedding의 추가 이득은 오류 유형에 따라 달랐습니다.
...
Embedding 결합은 모든 조건에서 좋아지지 않았습니다. 문서 전체가 훼손된 경우와 숫자·단어 하나만 바뀐 local critical error는 서로 다른 탐지 문제였습니다. 전체 의미가 유지되면 embedding과 document-level confidence가 정상이어도 중요한 field 하나는 틀릴 수 있습니다. 금액, 금리, 날짜처럼 한 자리 오류가 후속 판단을 바꾸는 field는 문서 전체 점수와 별도로 다뤄야 합니다.
...
운영에서는 이 점수를 최종 정답 판정에 쓰지 않습니다. 낮은 위험 문서는 기존 흐름으로 보내고, 높은 위험 문서는 재인식·fallback·사람 검수 후보로 올리며, batch signal은 새 양식이나 공급처 변화가 한꺼번에 들어왔는지 확인하는 경보로 사용할 수 있습니다.
