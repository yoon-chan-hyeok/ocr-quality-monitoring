...
**OCR 신뢰도와 임베딩 이탈도를 비교해 사람이 먼저 확인할 문서의 순서를 정합니다.**
...
[문제](#왜-이-문제를-다뤘나) · [방법](#어떤-신호를-비교했나) · [결과](#실험에서-확인한-것) · [도구](#공개한-도구) · [실행](#빠르게-확인하기)
...
이 프로젝트에서는 정답 전사본이 아직 없는 시점에 사용할 수 있는 신호를 찾았습니다. OCR 모델이 내놓은 신뢰도와 정상 문서에서 얼마나 벗어났는지를 나타내는 임베딩 이탈도를 비교해 검수 순서를 정했습니다. 이 점수는 문서가 틀렸다고 판정하는 값이 아니라, 사람이 먼저 볼 대상을 고르는 기준입니다.

## 30초 요약

- OCR 신뢰도와 문서 임베딩 이탈도를 정답 없는 검수 우선순위 신호로 비교했습니다.
